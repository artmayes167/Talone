...
//            if let c = creationManager {
//                c.setLocation(city: city, state: state, country: loc[.country] ?? "USA", community: loc[.community] ?? "")
//            } else {
//                creationManager = PurposeCreationManager(type: NeedType(rawValue: categoryTextField.text?.lowercased() ?? "none") ?? .none, city: city, state: state)
//            }
//            
...
//        guard let c = creationManager, let loc = c.getLocationOrNil() else {
//            view.makeToast("Failed to save Search Location in saveFor. creationManager = \(String(describing: creationManager)), loc = \(String(describing: creationManager.getLocationOrNil()))")
//            return
//        }
//        if !(type == .none) {
